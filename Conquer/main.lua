...
--Only player is dynamic, to be able to detect collision
...
local EndText = display.newText( "", display.contentCenterX, display.contentCenterY - 90, native.systemFont, 100 )
...
--local RushDurationText = display.newText("[4] - higher rush speed", display.contentCenterX, 25, native.systemFont, 12)
...
DeadScreenForeGround:setFillColor( 0, 0, 0, 0)

--called when a key is pressed
function keyHandler(event)
    --saves if a key is pressed down or not
...
--Button presses
    ...
    --makes current displacement speed "speedMultiplier" times the default if given buttons are pressed
...
function Shoot()
    ...
    --print( "rotation: "..(playerRotation + 270))
    --print( "x: "..math.cos(math.rad(playerRotation + 270)) )
    --print( "y: "..math.sin(math.rad(playerRotation + 270)) )
...
--crates in random position and sizes within screen limits
function createObject(type)
    ...
    if(type == "enemy") then r = math.random(3, 7) end
...
--We add static collider to detect collision
...
--(event.object1 and event.object2 collided in this engine)
...
--makes these appear before others
...
--travels all enemies and places them front since enemyID stores next id to be used, its also array.length
